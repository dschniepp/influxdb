...
type mockResourceShower struct {
	ShowDatabaseFn func(string) bool
}

func (rs *mockResourceShower) ShowDatabase(name string) bool {
	return rs.ShowDatabaseFn(name)
}
...
func TestQueryExecutor_ExecuteQuery_ShowDatabases(t *testing.T) {
	...
	opt := influxql.ExecutionOptions{
		ResourceShower: &mockResourceShower{
			ShowDatabaseFn: func(name string) bool {
